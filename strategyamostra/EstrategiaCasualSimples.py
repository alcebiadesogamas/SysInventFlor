from strategyestatistica.StrategySimple import estat as estatisticas
from model.Tabela import Tabela
import pandas as pd
import matplotlib.pyplot as plt
import random
dados = pd.read_excel('resources/pop.xlsx')
...
def excuteAll(n, nsim, nst):
    ...
    valor_t = tb.setValoresTtabelado(nst, n)

    # Calcula as estatísticas para cada simulação
    result = estatisticas.estat(lista2, variavel, nsim, valor_t[0])
    lmedias = []
    for i in range(0, len(result)):
